...
audio_file_mimelist = ['audio/mpeg', 'audio/x-mpeg-3', 'audio/mpeg3', 'audio/aiff', 'audio/x-aiff', 'audio/m4a',
...
class GoSyncModel(object):
    def __init__(self):
        ...
        self.iobserv_handle = self.observer.schedule(FileModificationNotifyHandler(self), self.mirror_directory,
                                                     recursive=True)
    ...
    def UploadObservedFile(self, file_path):
        if self.IsSyncRunning():
            self.SendlToLog(3,
                            "UploadObservedFile: File %s is created but sync is running. Possibly created by sync. Skipping."
                            % file_path)
            return
    ...
    def LocateFileOnDrive(self, abs_filepath):
        ...
        if dirpath != '':
            ...
            try:
                ...
                self.SendlToLog(1,
                                "LocateFileOnDrive: Unknown error in locating file (%s) in local folder (root)" % filename)
                raise
                ...
                while True:
                    ...
                    try:
                        ...
                        if os.path.exists(dirpath) and os.path.isfile(dirpath):
                            self.SendlToLog(2,
                                            "SyncLocalDirectory: Uploading Local File (%s) - Not in Remote\n" % dirpath)
                            GoSyncEventController().PostEvent(GOSYNC_EVENT_SYNC_UPDATE, {
            ...
            for names in dirs:
                ...
                try:
                    ...
                    if os.path.exists(dirpath) and os.path.isdir(dirpath):
                        self.SendlToLog(3,
                                        "SyncLocalDirectory: Uploading Local Folder (%s) - Not in Remote\n" % dirpath)
                        try:
                            GoSyncEventController().PostEvent(GOSYNC_EVENT_SYNC_UPDATE,
                                                              {"Creating Folder: %s" % f['name']})
            ...
            try:
                ...
                if not self.IsInternetReachable():
                    ...
                    if retry == 0:
                        ...
                        self.SendlToLog(1, "MakeFileListQuery (unknown except %d) - Raising FileListQueryFailed",
                                        e.errno)
            ...
            for f in file_list:
                ...
                if f['mimeType'] == 'application/vnd.google-apps.folder':
                    ...
                    if not self.IsGoogleDocument(f):
                        ...
                        self.SendlToLog(3,
                                        "SyncRemoteDirectory: Skipping file (%s) is a google document.\n" % f['name'])
        except InternetNotReachable:
...
class FileModificationNotifyHandler(PatternMatchingEventHandler):
    ...
    def on_moved(self, evt):
        self.sync_handler.logger.info(
            "Observer: file %s moved to %s: Not supported yet!\n" % (evt.src_path, evt.dest_path))
